...
def fetch_data_from_database(url_path, supabase):
    ...
    response = supabase.table('LocalityDiscount').select("locality_id", "name_of_option", "degree_of_disability", "mark_ag", "mark_b", "mark_bl", "mark_h", "mark_gl", "mark_g", "mark_tb", "standard_price", "discounted_price", "companion_price").eq("locality_id", locality_id).execute()
